...
int search_city(DATABASE* db, NAME_TYPE newCityName[], COORDS_TYPE newCityCoords[], int dbLength, int isDelete){
    newCityCoords[0] = 0;
    newCityCoords[1] = 0;
    char option = '0';
    ...
    switch (option){
        ...
        search_city(db, newCityName, newCityCoords, dbLength, 0);
    }
    ...
    if(cityPosition != INVALID && isDelete == 1) return cityPosition;
...
void list_distance_cities(int dbLength, DATABASE* db){
    ...
    for(int i = 0; i < dbLength; ++i){
        long double qua1 = pow(db->city[i].coords[0] - CityCoords[0], 2);
        long double qua2 = pow(db->city[i].coords[1] - CityCoords[1], 2);
